...
TPC-H scale factor 1 (6M lineitem rows) on Apple M4 Pro (14-core, 192 KB L1D per performance core, 36 MB SLC). Page cache warm. Each benchmark run 10×; numbers are median wall time. The harness reports per-iteration min/median/max alongside the mean (`min-ms`/`med-ms`/`max-ms` in the benchmark output), so the spread behind any headline margin is visible in the same line that produced it — load-bearing for the Q1 headline, whose margin against DuckDB's steady-state is small enough that background load can move it. Iteration counts per engine: vexq and SQLite run at `-benchtime=10x`; the DuckDB rows were collected at `-benchtime=3x` (in-harness) and via `.timer` medians (CLI) — a difference in sample count, disclosed here rather than hidden. All ratios are computed from unrounded medians, so a ratio may differ slightly from one recomputed from the rounded milliseconds shown. The headline results: on Q1, vexq parallel (22 ms median, 21–24 ms spread) matches DuckDB's cold-start latency (24 ms first invocation) and sits within 1.2× of its steady-state (19 ms warm page cache), within 1.2× single-threaded per core; vexq is faster than SQLite on all four queries (3.0–18.4×); see the DuckDB decomposition below.
...
All four TPC-H query results verified identical to SQLite output (via the in-harness `TestQ*Correctness` assertions; Q6's SUM is asserted to 1e-9 relative tolerance). vexq's canonical Q6 additionally matches DuckDB's result exactly.

Beyond those four queries, the evidence is test coverage, and it extends only as far as the tests do. A 107-query golden suite ([`internal/goldentest/`](../internal/goldentest/)) compares results with a naive row-at-a-time reference evaluator on a small generated dataset. Each query runs through four paths (serial, `planner.Parallel(…, 4)`, optimizer-off, and a stacked-filter plan that duplicates pushed-down predicates), and the suite checks output width and aliases as well as rows. An error on the parallel path fails the case. `planner/sql_semantics_test.go` pins hand-computed answers, serial and parallel, for behavior the reference evaluator cannot check, such as ORDER BY on unselected columns and the constructs [sql.md](sql.md) lists as rejected. All of these pass, including under `-race`.

Passing these tests is not a proof of correctness. This section previously said a 72-query suite verified "the full SQL subset" with "zero known correctness issues". A later review found several SQL behaviors the suite never exercised that returned wrong results: dropped join predicates, confused same-named columns, wrong `IN`/`NOT IN` results, extra or reordered aggregate columns, ignored `OFFSET`/`UNION`/`HAVING`, and an `fsck` that never checked zone maps. The harness had also been silently skipping the parallel comparison whenever that path returned an error. All of those are fixed with regression tests, as are three bugs found while fixing them: `ORDER BY` misordered negative floats, zone maps mis-pruned negative-float predicates, and division by zero returned 0. Treat SQL shapes the tests do not cover as unverified.
...
- **Parallelism gap (closed):** DuckDB achieves 3.6–6.4× scaling from threads=1 to threads=14 (from the table above: Q1 6.4×, Q6 4.6×, Q12 5.1×, Q3 3.6×). vexq now achieves 8.6× (Q1), 6.7× (Q12), 6.5× (Q6), and 3.8× (Q3) — exceeding DuckDB's scaling on all four queries (Q3: 3.8× vs 3.65×). One structural caveat on that comparison: scaling ratios systematically favor the engine with the slower serial baseline — vexq scales from 193 ms and DuckDB from 159 ms on Q1, so the same fixed per-query overheads occupy a smaller fraction of vexq's serial time, leaving more parallelizable work to divide. The end-to-end crossover is a same-machine fact; the scaling ratios alone are not evidence of better parallel engineering per se. All four take the parallel path; the GOGC ablation below shows GC — formerly 26–50% of parallel wall time — is now 0–19% after the allocation campaign.
...
- **Radix partitioning did not make the probe cache-resident**: this was the stated motivation and the measurement did not support it. `exec.BenchmarkPartitionedProbe` holds the build rows and probe keys fixed and varies only the partition count: 1 → 256 partitions is flat within noise (4.1–4.7 ms). Only the build side is partitioned, so a probe batch's keys still spread across every partition and the probe touches the whole table per unit time either way. What partitioning does buy is the lock-free parallel build, plus 1.58× on a single-threaded build purely from growing 64 small maps instead of one big one (`exec.BenchmarkRadixBuild`). Getting the cache win needs the **probe** stream partitioned too — buffering probe rows per partition at morsel granularity and then probing partition-at-a-time — which conflicts with a streaming operator whose `TableScan` reuses its decode buffers between batches. That is the remaining structural piece of DuckDB's join advantage, and it is not claimed here. The premise was measured on 2026-08-20 from the time-share side (hardware counters require full Xcode, not installed on the benchmark machine): a CPU profile over 10 iterations each of Q3 and Q12 shows probe lookups do not register above pprof's sampling threshold — under ~0.5% of combined wall time — while the visible join cost is entirely build-side (`joinHashTable.insert` ~6% flat, `rowStore` writes ~9% cumulative) and both queries remain I/O-dominated (pread syscalls 50%, `madvise` 18%). A time share that small bounds any probe-partitioning win at ~1.005× regardless of miss rate, so probe-stream partitioning is **declined as measured**. (Q3's filtered customer side, ~30K rows, also fits in the 36 MB SLC and could not have benefited regardless.)
- **Column pruning through joins is name-based, not cost-based**: needed-column sets carry plan-unique names (a column name shared by several tables is qualified by its table), so each side decodes exactly the columns the query reads from it. A scan cannot express "read zero columns" — `SELECT COUNT(*) FROM t` over a single table still decodes every column, because an empty projection means "all" in `exec.NewTableScan`. Under a join the join key saves it, but the single-table case would need a row-count-only scan path.
...
# Load SQLite (rebuilds data/tpch.db; skipped unless explicitly enabled)
